...
def process_sheet(sheet):
    ...
    for row in sheet.iter_rows(min_row=2, max_col=17):
        if not row[4].value:  # Stop if we reach an empty row in column E
            break
        index+=1
        weight = row[4].value  # Column E
        param = row[5].value   # Column F
        value = row[6].value   # Column G
        thresholds = [cell.value for cell in row[7:12]]  # Columns H to L
    ...
    last_row = index + 2
    final_score_cell = sheet.cell(row=last_row, column=13)  # Column M
    final_score_cell.value = final_score
    final_score_cell.font = Font(bold=True)
    sheet.cell(row=last_row, column=6, value="Final Score:").font = Font(bold=True)
...
def group_parameters_by_section(sheet):
    ...
    for row in sheet.iter_rows(min_row=2, max_col=6):
        if row[3].value:  # Column D
            current_section = row[3].value
            sections[current_section] = []
        if row[5].value and current_section:  # Column F (parameter)
            sections[current_section].append(row[5].value)
...
def main():
    file_path = 'python_template.xlsx'  # Replace with your actual file path
    ...
    plot_scatter([score for score, _ in scores_with_names], plots)
